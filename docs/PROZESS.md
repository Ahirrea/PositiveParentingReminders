...
betroffen? Ehrlich benennen, was wiederverwendbar ist und was fehlt. (Beispiel Journal:
der Onboarding-Flow und die Lottie-Einbindung stehen schon, `JournalEditorActivity`
ist ein Stub ohne `setContentView` und es gibt noch keine Persistenz.)

### 3. Spannungen & Randbedingungen sichtbar machen
Gegen die Projekt-DNA prüfen: [`PRD.md`](./PRD.md) (Ziele/**Nicht-Ziele**), die
[Entscheidungen](./entscheidungen/README.md), die Konventionen in `CLAUDE.md`.
Widerspricht die Idee einem Nicht-Ziel? Das ist kein K.-o., aber es muss
**explizit** benannt und aufgelöst werden. Das stärkste Nicht-Ziel hier ist
„kein Backend, keine Cloud, kein Konto" — alles, was Einträge das Gerät verlassen
lässt, braucht eine ausdrückliche Umkehrung als eigener ADR.
...
- Tests grün (`./gradlew test`), inkl. neuer Tests für neue Logik — heute existiert noch keine Test-Suite, neue Anforderungen legen sie mit an.
- **Neuer Screen ist im `AndroidManifest.xml` registriert** (`exported="false"`, außer Launcher/Deep-Link) — sonst ist er nicht erreichbar.
- Keine hartkodierten UI-Strings: Texte nach `res/values/strings.xml`, Farben nach `colors.xml`. Neue Abhängigkeiten über `gradle/libs.versions.toml`.
