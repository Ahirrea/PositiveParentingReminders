...
→ `OnboardingStep3Activity` → `AccountCreationActivity`) und seit A-1 der
**Journal-Editor** (`journal/JournalEditorActivity` mit Room-Persistenz unter
`data/`); beide sind im `AndroidManifest.xml` registriert. Der Rest ist Gerüst:
...
Mit A-1 existiert das Fundament (lokal gespeicherte Einträge); alles Weitere
baut darauf auf.
...
|---|---|---|---|
| A-1 | [Eintrag schreiben und speichern](./A-1-eintrag-schreiben-und-speichern.md) | 🏁 erledigt | Der Kern des Produkts: Tagesimpuls, Textfeld, Stimmung, Speichern — lokal in Room ([ADR-004](../entscheidungen/ADR-004-room-als-lokale-persistenz.md)). Bedient PRD „Kernschleife" Schritte 1–2. |
